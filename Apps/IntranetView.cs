...
public sealed class IntranetView : UserControl, IDisposable
{
    ...
    private readonly TextBox _address = new() { Text = "lumon.internal/home", Name = "BrowserAddress" };
    ...
    public IntranetView(SystemState state)
    {
        ...
        state.Changed += RenderPage; Navigate("lumon.internal/home");
    }
    public void Navigate(string address)
    {
        address = address.Trim(); if (!address.Contains('/')) address = address is "home" or "handbook" or "departments" or "announcements" ? "lumon.internal/" + address : address + "/home";
    ...
    private void RenderPage()
    {
        ...
        var address = _position >= 0 ? _history[_position] : "lumon.internal/home";
        ...
        switch (CurrentPage)
        {
            case "home":
                ...
                stack.Children.Add(Paragraph("Here, every detail has a purpose. Every person has a place. Welcome to your corner of Lumon Industries."));
                stack.Children.Add(Ui.Button("Explore your department  →", () => Navigate("departments"), "primary"));
                stack.Children.Add(Ui.Rule());
                stack.Children.Add(Ui.Columns("*,*", Ui.Stack(10, Ui.Label("01 / COMPANY NEWS"), Ui.Text("A productive quarter.", 21), Paragraph("The board extends its appreciation to all refiners.")), Ui.Stack(10, Ui.Label("02 / EMPLOYEE RESOURCES"), Ui.Text("A little guidance.", 21), Ui.Button("Read the nine principles →", () => Navigate("handbook"), "outline"))));
                break;
            case "departments":
                ...
                foreach (var (name, description) in new[] { ("Macrodata Refinement", "Finding order. Bringing balance. Your assigned department."), ("Optics & Design", "Carefully considered objects for a carefully considered world."), ("Wellness", "The continued harmony of our valued employees."), ("Administration", "A steady hand. An enduring commitment.") }) { stack.Children.Add(Ui.Stack(8, Ui.Text(name, 23), Paragraph(description))); stack.Children.Add(Ui.Rule()); }
                break;
            case "announcements":
                stack.Children.Add(Ui.Heading("THE DEPARTMENT BULLETIN", "Good things are happening.", "Approved updates for the severed floor."));
                ...
                break;
            case "handbook":
                stack.Children.Add(Ui.Heading("THE NINE PRINCIPLES", "The foundation of our work.", "Let these qualities guide your every contribution."));
                foreach (var p in new[] { "Vision", "Verve", "Wit", "Cheer", "Humility", "Benevolence", "Nimbleness", "Probity", "Wiles" }) stack.Children.Add(Ui.Text(p, 24));
                break;
            default:
        ...
        stack.Children.Add(Ui.Rule()); stack.Children.Add(Ui.Label("LUMON INDUSTRIES  ·  INTERNAL USE  ·  SEVERED FLOOR"));
