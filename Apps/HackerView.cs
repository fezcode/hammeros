...
public sealed class HackerView : UserControl, IDisposable
{
    ...
    public HackerView()
    {
        ...
        var right = Ui.Stack(20, Ui.Label("SIGNAL RECONSTRUCTION", Ui.Muted), _metric, _progress, Ui.Rule(Tint.DarkLine), Ui.Label("TRAINING NODES", Ui.Muted), Ui.Text("192.0.2.14    ONLINE\n192.0.2.27    RELAY\n198.51.100.8  STANDBY", 12, Ui.Phosphor, true), _field);
        var body = Ui.Columns("*,220", new Border { Padding = new Thickness(24, 12), Child = _feed, ClipToBounds = true }, new Border { Padding = new Thickness(18), BorderBrush = Tint.DarkLine, BorderThickness = new Thickness(1, 0, 0, 0), Child = right }); Grid.SetRow(body, 1); root.Children.Add(body);
