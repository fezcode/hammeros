...
public sealed class DepartmentTerminalView : UserControl
{
    ...
    private readonly TextBlock _output = Ui.Text("HAMMEROS [VERSION 0.1.0]\n(c) Hammer Industries. All work is valued.\n\nSecure session established.\nWelcome back, refiner.\n", 12, Ui.Phosphor, true);
    ...
    public DepartmentTerminalView(SystemState state, Action<string> open)
    {
        ...
        Input.Classes.Add("terminal"); Input.FontSize = 12; Input.Padding = new Thickness(6, 5);
        ...
        var command = Ui.Columns("Auto,*", _prompt, Input); command.Margin = new Thickness(20, 0, 16, 12); Grid.SetRow(command, 1); grid.Children.Add(command);
        var footer = new Border { Padding = new Thickness(20, 0), Background = Avalonia.Media.Brush.Parse("#193A40"), Child = Ui.Columns("*,Auto", Ui.Label("HSH  /  VIRTUAL SHELL", Ui.Muted), Ui.Label("MDR-04   ●", Ui.Phosphor)) }; Grid.SetRow(footer, 2); grid.Children.Add(footer);
        Content = grid; Prompt();
        Input.KeyDown += (_, e) =>
        {
    ...
    private void Prompt() => _prompt.Text = $"mdr-04:{Session.Cwd.Replace("/department", "~")} $";
