...
/// <summary>
/// Column width of Unicode scalars for a monospace grid: 0 for controls, combining
/// marks and format characters, 2 for East Asian Wide/Fullwidth and emoji with
/// default emoji presentation, 1 otherwise. Tables are inclusive [lo, hi] ranges
/// searched with binary search; they cover what real shells and TUIs emit rather
/// than the whole Unicode database.
/// </summary>
public static class Wcwidth {
    static readonly (int Lo, int Hi)[] ZeroWidth = [
        ...
        (0x0A41, 0x0A51), (0x0A70, 0x0A71), (0x0A75, 0x0A75), (0x0A81, 0x0A82), (0x0ABC, 0x0ABC),
        (0x0AC1, 0x0AC8), (0x0ACD, 0x0ACD), (0x0B01, 0x0B01), (0x0B3C, 0x0B3C), (0x0B3F, 0x0B3F),
        (0x0B41, 0x0B44), (0x0B4D, 0x0B56), (0x0BC0, 0x0BC0), (0x0BCD, 0x0BCD), (0x0C3E, 0x0C40),
        (0x0C46, 0x0C56), (0x0CBC, 0x0CBC), (0x0CCC, 0x0CCD), (0x0D41, 0x0D44), (0x0D4D, 0x0D4D),
        (0x0DCA, 0x0DCA), (0x0DD2, 0x0DD6), (0x0E31, 0x0E31), (0x0E34, 0x0E3A), (0x0E47, 0x0E4E),
        ...
        (0x0F8D, 0x0FBC), (0x0FC6, 0x0FC6), (0x102D, 0x1030), (0x1032, 0x1037), (0x1039, 0x103A),
        (0x103D, 0x103E), (0x1058, 0x1059), (0x105E, 0x1060), (0x1071, 0x1074), (0x1082, 0x1082),
        (0x1085, 0x1086), (0x108D, 0x108D), (0x109D, 0x109D), (0x135D, 0x135F), (0x1712, 0x1714),
        (0x1732, 0x1734), (0x1752, 0x1753), (0x1772, 0x1773), (0x17B4, 0x17B5), (0x17B7, 0x17BD),
        (0x17C6, 0x17C6), (0x17C9, 0x17D3), (0x17DD, 0x17DD), (0x180B, 0x180E), (0x1885, 0x1886),
        (0x18A9, 0x18A9), (0x1920, 0x1922), (0x1927, 0x1928), (0x1932, 0x1932), (0x1939, 0x193B),
        (0x1A17, 0x1A18), (0x1A1B, 0x1A1B), (0x1A56, 0x1A56), (0x1A58, 0x1A60), (0x1A62, 0x1A62),
        ...
        (0x1CF8, 0x1CF9), (0x1DC0, 0x1DFF), (0x200B, 0x200F), (0x2028, 0x202E), (0x2060, 0x2064),
        ...
        (0xFFF9, 0xFFFB), (0x101FD, 0x101FD), (0x102E0, 0x102E0), (0x10376, 0x1037A), (0x10A01, 0x10A0F),
        (0x10A38, 0x10A3F), (0x10AE5, 0x10AE6), (0x10D24, 0x10D27), (0x10EAB, 0x10EAC), (0x10F46, 0x10F50),
        ...
        (0x111B6, 0x111BE), (0x1D167, 0x1D169), (0x1D17B, 0x1D182), (0x1D185, 0x1D18B), (0x1D1AA, 0x1D1AD),
        (0x1D242, 0x1D244), (0x1F3FB, 0x1F3FF), (0xE0001, 0xE0001), (0xE0020, 0xE007F), (0xE0100, 0xE01EF),
    ];

    static readonly (int Lo, int Hi)[] Wide = [
    ...
    public static int Width(Rune r) {
        ...
        if (InTable(ZeroWidth, cp)) return 0;
        if (InTable(Wide, cp)) return 2;
        return 1;
    }

    /// <summary>True for printable scalars that occupy no column (combining marks, ZWJ, selectors) — not for controls.</summary>
    public static bool IsZeroWidth(Rune r) => r.Value >= 0x300 && Width(r) == 0;
    public static bool IsZwj(Rune r) => r.Value == 0x200D;
    public static bool IsVariationSelector(Rune r) =>
        (r.Value >= 0xFE00 && r.Value <= 0xFE0F) || (r.Value >= 0xE0100 && r.Value <= 0xE01EF);

    /// <summary>Width of a grapheme cluster: that of its first non-zero-width scalar, or 1 if every scalar is zero-width.</summary>
    public static int ClusterWidth(string cluster) {
        foreach (var r in cluster.EnumerateRunes()) {
            var w = Width(r);
            if (w > 0) return w;
        }
        return 1;
    ...
    static bool InTable((int Lo, int Hi)[] table, int cp) {
